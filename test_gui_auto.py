...
class AutoTestTool:
    ...
    def _create_widgets(self):
        ...
        # 自动化按钮
        ttk.Separator(left_frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=5)
        self.btn_auto = ttk.Button(left_frame, text="自动运行测试", command=self._auto_run)
        ...
        self.log_text.tag_configure("detail", foreground="#666666")

        # 底部 - 状态栏
        bottom_frame = ttk.Frame(self.root, padding=(10, 5))
        bottom_frame.pack(fill=tk.X)

        self.status_var = tk.StringVar(value="就绪 - 点击'自动运行测试'开始")
    ...
    def _send_request(self, command, username):
        ...
        try:
            ...
            return {"replies": "[连接失败 - 请检查AstrBot是否运行]", "raw_messages": [], "status_code": 0}
        except requests.exceptions.Timeout:
            return {"replies": "[请求超时]", "raw_messages": [], "status_code": 0}
        except Exception as e:
            return {"replies": f"[错误: {e}]", "raw_messages": [], "status_code": 0}
    ...
    def _test_command(self, name, command):
        ...
        result = self._send_request(command, username)

        if isinstance(result, dict):
            response = result["replies"]
            raw = result.get("raw_messages", [])
        ...
        if raw:
            ...
            type_str = ", ".join([f"{k}:{v}" for k, v in msg_types.items()])
            self._log(f"  [原始消息: {type_str}]", "detail")
