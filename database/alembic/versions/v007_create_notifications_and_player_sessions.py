...
revision: str = "v007"
down_revision: Union[str, Sequence[str], None] = "v006"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("target_type", sa.Text, nullable=False, server_default="all", comment="目标类型: all/specific/realm"),
        sa.Column("target_ids", sa.Text, nullable=True, comment="目标ID列表(JSON)"),
        sa.Column("sender_id", sa.Text, nullable=True, server_default="system", comment="发送者ID"),
        ...
        sa.Column("status", sa.Text, server_default="pending", comment="状态: pending/sending/completed/failed"),
    )

    op.create_table(
        "player_sessions",
        sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
        sa.Column("user_id", sa.Text, nullable=False, unique=True, comment="玩家用户ID"),
        sa.Column("unified_msg_origin", sa.Text, nullable=False, comment="统一消息来源标识(platform_id:message_type:session_id)"),
        sa.Column("platform_name", sa.Text, nullable=True, comment="平台名称"),
        sa.Column("updated_at", sa.TIMESTAMP, server_default=sa.text("CURRENT_TIMESTAMP"), comment="最后更新时间"),
    )
