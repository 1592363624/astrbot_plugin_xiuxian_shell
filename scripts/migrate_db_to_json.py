...
运行方式：
python scripts/migrate_db_to_json.py

数据将导出到插件目录：data/game_data/
这个目录可以纳入版本控制

此脚本会：
1. 读取插件数据目录
2. 如果数据库存在，直接从数据库导出数据
3. 如果数据库不存在，使用代码中定义的默认数据创建JSON文件
"""
...
JSON_DIR = PLUGIN_DIR / "data" / "game_data"
DB_PATH = PLUGIN_DIR / "data" / "plugin_data" / "astrbot_plugin_xiuxian_shell" / "xiuxian.db"
...
DEFAULT_DATA = {
    ...
    "items": [
        ...
        {"id": "item_004", "name": "清灵丹", "description": "清除体内丹毒", "item_type": "consumable", "rarity": "rare", "effect_type": "detox", "effect_value": 0, "price": 300, "is_usable": True, "realm_requirement": "realm_006"},
        {"id": "item_005", "name": "筑基丹", "description": "增加突破筑基成功率", "item_type": "consumable", "rarity": "epic", "effect_type": "breakthrough", "effect_value": 20, "price": 1000, "is_usable": True, "realm_requirement": "realm_005"},
        {"id": "item_006", "name": "破境丹", "description": "增加突破境界概率", "item_type": "consumable", "rarity": "legendary", "effect_type": "breakthrough", "effect_value": 30, "price": 5000, "is_usable": True, "realm_requirement": "realm_010"},
        {"id": "item_007", "name": "灵草", "description": "炼制丹药的原料", "item_type": "material", "rarity": "common", "effect_type": None, "effect_value": 0, "price": 20, "is_usable": False, "realm_requirement": None},
        {"id": "item_008", "name": "百年灵芝", "description": "珍贵的炼丹药材", "item_type": "material", "rarity": "rare", "effect_type": None, "effect_value": 0, "price": 500, "is_usable": False, "realm_requirement": None},
    ],
    "skills": [
...
def migrate_data():
    ...
    if DB_PATH.exists():
        ...
        for table, json_name in [("realms", "realms"), ("items", "items"), ("skills", "skills"), ("game_events", "events")]:
            try:
                ...
                if not rows:
                    ...
                    data = DEFAULT_DATA.get(json_name, [])
                else:
                    data = []
                    for row in rows:
                        item = dict(row)
                ...
                print(f"导出 {table} -> {json_name}.json: {len(data)} 条记录")
        ...
        print(f"\n数据库文件不存在，使用默认数据创建JSON文件...\n")

        migration_results = {}
        for json_name, data in DEFAULT_DATA.items():
            json_path = JSON_DIR / f"{json_name}.json"
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
            print(f"创建 {json_name}.json: {len(data)} 条记录")
            migration_results[json_name] = {"status": "success", "count": len(data)}
    ...
    for name, result in migration_results.items():
        if result["status"] == "success":
            print(f"  ✅ {name}: {result['count']} 条记录")
        else:
            print(f"  ❌ {name}: {result.get('error', '未知错误')}")

    print(f"\nJSON文件位置: {JSON_DIR}")
    print("\n这些JSON文件可以纳入版本控制（git）。")
